...
/** Seeds a pending Uzum subscription payment for Merchant/Checkout E2E scripts. */
export const seedUzumPayment = internalMutation({
  ...
  handler: async (ctx, args) => {
    const now = Date.now();
    const email = args.email ?? `uzum-e2e-${now}@test.local`;
    const clerkId = args.clerkId ?? `e2e-uzum-${now}`;
