import { withDatabase, withUserOrAdminDatabase } from "@/lib/db/runtime";
...
export async function createOrganizationForUser(
  ...
  const existing = await getUserOrganizationDetails(userId, accessToken);
  if (existing) {
  ...
  const organization = await getUserOrganizationDetails(userId, accessToken);
  return { organization };
}
...
export async function updateOrganizationForUser(
  ...
  return getUserOrganizationDetails(userId, accessToken);
}
